...
$TOOL_GIT_REPO = "https://github.com/rBrenick/script-panel/archive/master.zip" # modify these after uploading the tool
$UPDATE_PATHS = @("docs", "script_panel", "README.md", "setup.py")

$CURRENT_TOOL_FOLDER = (Get-Item -Path ".").FullName
$CURRENT_script_panel = (Get-Item -Path ".").Name

$NEW_VERSION_ZIP = $USER_DOWNLOADS_FOLDER + "\" + $CURRENT_script_panel + "_update.zip"
$NEW_VERSION_ZIP_FOLDER = $USER_DOWNLOADS_FOLDER + "\" + $CURRENT_script_panel + "_update"
...
foreach ($element in $UPDATE_PATHS) {
    ...
    $src_file_path = "$NEW_VERSION_ZIP_FOLDER\$CURRENT_script_panel-master\$element"
